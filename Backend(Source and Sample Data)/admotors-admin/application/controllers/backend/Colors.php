...
class Colors extends BE_Controller {
	...
	function is_valid_input( $id = 0 ) {
		...
		$this->form_validation->set_rules( 'color_value', get_msg( 'color_value' ), $rule);
	...
	{		
		 ...
		 $conds['color_value'] = $name;

			
		 	if( $id != "") {
		 		// echo "bbbb";die;
				if ( strtolower( $this->Color->get_one( $id )->color_value ) == strtolower( $name )) {
	...
	function edit( $id ) {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'color_edit' );
	...
	/**
	 * Delete all the news under color
	 *
	 * @param      integer  $color_id  The color identifier
	 */
	function delete( $color_id = 0 )
	{
		// start the transaction
		$this->db->trans_start();
		...
		/** Note: enable trigger will delete news under color and all news related data */
		if ( !$this->ps_delete->delete_color( $color_id, $enable_trigger )) {
		// if error in deleting color,
	...
	function ajx_exists( $id = false )
	{
		...
		$name = $_REQUEST['color_value'];

		if ( $this->is_valid_name( $name, $id )) {

		// if the color name is valid,
			
			echo "true";
		} else {
		// if invalid color name,
	 ...
	 */
	function ajx_publish( $color_id = 0 )
	{
		// check access
		$this->check_access( PUBLISH );
		
		// prepare data
		$color_data = array( 'status'=> 1 );
	 ...
	 * @param      integer  $manufacturer_id  The manufacturer identifier
	 */
	function ajx_unpublish( $color_id = 0 )
	{
		// check access
		$this->check_access( PUBLISH );
		
		// prepare data
		$color_data = array( 'status'=> 0 );
