<div class="table-responsive animated fadeInRight">
	<table class="table m-0 table-striped">
		...
		<?php if ( !empty( $rejects ) && count( $rejects->result()) > 0 ): ?>
			...
			<?php foreach($rejects->result() as $rejcet): ?>
				
				<tr>
					<td><?php echo ++$count;?></td>
					<td><?php echo $rejcet->wallpaper_name;?></td>
					<td><?php echo $this->Category->get_one($rejcet->cat_id)->cat_name;?></td>

					<?php $default_photo = get_default_photo( $rejcet->wallpaper_id, 'wallpaper' ); ?>	
					...
					<td>
						<?php echo $rejcet->point; ?>
					</td>

					<?php if ( $this->ps_auth->has_access( EDIT )): ?>
				
						<td>
							<a href='<?php echo $module_site_url .'/edit/'. $rejcet->wallpaper_id; ?>'>
