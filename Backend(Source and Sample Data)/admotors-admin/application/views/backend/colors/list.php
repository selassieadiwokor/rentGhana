	...
	<?php if ( !empty( $colors ) && count( $colors->result()) > 0 ): ?>
		...
		<?php foreach($colors->result() as $col): ?>
			...
			<tr>
				...
				<td ><?php echo $col->color_value; ?></td>
				...
				<?php if ( $this->ps_auth->has_access( PUBLISH )): ?>
					...
					<td>
						<?php if ( @$col->status == 1): ?>
							<button class="btn btn-sm btn-success unpublish" id='<?php echo $col->id;?>'>
							Yes</button>
						<?php else:?>
							<button class="btn btn-sm btn-danger publish" id='<?php echo $col->id;?>'>
							No</button><?php endif;?>
					</td>
