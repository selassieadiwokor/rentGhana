<div class="table-responsive animated fadeInRight">
	<table class="table m-0 table-striped">
		<tr>
			...
			<th><?php echo get_msg('type_name'); ?></th>
	...
	<?php if ( !empty( $types ) && count( $types->result()) > 0 ): ?>
		...
		<?php foreach($types->result() as $type): ?>
			...
			<tr>
				...
				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
					...
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $type->id; ?>'>
				...
				<?php if ( $this->ps_auth->has_access( PUBLISH )): ?>
					...
					<td>
						<?php if ( @$type->status == 1): ?>
							<button class="btn btn-sm btn-success unpublish" id='<?php echo $type->id;?>'>
							Yes</button>
						<?php else:?>
							<button class="btn btn-sm btn-danger publish" id='<?php echo $type->id;?>'>
							No</button><?php endif;?>
					</td>
